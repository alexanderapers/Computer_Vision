...
#include "../background/BackgroundSubtraction.h"
...
Scene3DRenderer::Scene3DRenderer(
		Reconstructor &r, const vector<Camera*> &cs, const vector<BackgroundSubtraction*> &bgs) :
				m_reconstructor(r),
				m_cameras(cs),
				m_bg_subtraction(bgs),
				m_num(4),
...
bool Scene3DRenderer::processFrame()
{
	for (size_t c = 0; c < m_cameras.size(); ++c)
	{
		...
		m_bg_subtraction[c]->processForeground(this, m_cameras[c]);
	}
	return true;
}
